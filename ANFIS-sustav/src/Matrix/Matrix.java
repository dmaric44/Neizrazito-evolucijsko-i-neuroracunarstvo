...
public class Matrix {
	...
	public Matrix multiply(Matrix M) {
		...
		for(int i=0; i<rows; i++) {
			for(int j=0; j<columns; j++) {
				N.setData(i, j, data[i][j] * M.getData(i, j));
			}
		}
		return N;
	}
	...
	public static Matrix vector(double...elements) {
		Matrix M = new Matrix(elements.length, 1);
		for(int i=0; i<M.rows; i++) {
			M.setData(i, 0, elements[i]);
		}
		return M;
	...
	public Matrix replicateAddColumn() {
		Matrix N = new Matrix(rows, columns+1);
		for(int i=0; i<rows; i++) {
			N.setData(i, 0, getData(i,0));
			N.setData(i, 1, getData(i,0));
		}
		return N;
	}
}
